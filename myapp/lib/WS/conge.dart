...
class _CongeState extends State<Conge> {
  ...
  createcancelDialog(BuildContext context, String ooid) {
    Color gr = HexColor("#05445E");
    return AwesomeDialog(
      context: context,
      dialogType: DialogType.WARNING,
      animType: AnimType.SCALE,
      title: 'Avertissement :',
      desc: 'Cette demande va être Annulée',
      btnOkText: "Annuler",
      btnOkColor: Colors.grey[500],
      btnOkOnPress: () async {
        ///chanfgemet de l'etat de la demande de congé
        ConnectWS ws =
            ConnectWS("<OOOREQId>$ooid</OOOREQId>", "ReqStatusIdUpdate");
        await ws.canceldecision(context);
        //changement de etat de task
        ConnectWS ws2 =
            ConnectWS("<OOOREQId>$ooid</OOOREQId>", "TaskStatusIdUpdate");
        await ws2.canceldecision(context);
        refreshinglist();
            ...
            children: [
              Visibility(
                ...
                child: RaisedButton(
                    ...
                    onPressed: () {
                      return createcancelDialog(context, t.oooReqId);
                    }),
              ),
              RaisedButton(
                  child: Text(
      ...
      if (con.length != 0) {
        for (var i = 0; i < con.length; i++) {
          if (con[i].description.toLowerCase().contains(ch) ||
              frenshdate(con[i].beginDate.toLowerCase()).contains(ch) ||
              frenshdate(con[i].endDate.toLowerCase()).contains(ch) ||
